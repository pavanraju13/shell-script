...
echo -e "${B}Installing MySQL...${N}" | tee -a "$LOG_FILE"
dnf install mysql -y &>>"$LOG_FILE"
VALIDATE $? "Install MySQL"

# Install Git and validate
echo -e "${B}Installing Git...${N}" | tee -a "$LOG_FILE"
dnf install git -y &>>"$LOG_FILE"
VALIDATE $? "Install Git"

# Intentionally wrong command for demonstration
echo -e "${B}Installing Docker (intentionally wrong command)...${N}" | tee -a "$LOG_FILE"
